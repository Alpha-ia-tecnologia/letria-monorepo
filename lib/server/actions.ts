...
import { deriveStudentProgress, getWorldStatus, scoreActivity } from "@/lib/pedagogy";
...
function answersInput(value: unknown, questions: Question[]): Answers {
  ...
  for (const q of questions) {
    ...
    if (q.type === "choice") { if (typeof answer !== "string" || !q.options.includes(answer)) throw new ApiError(400, "Alternativa inválida."); result[q.id] = answer; }
    else { if (!Array.isArray(answer) || answer.length > q.options.length || answer.some((s) => typeof s !== "string" || !q.options.includes(s))) throw new ApiError(400, "Ordem de palavras inválida."); const counts = (arr: unknown[], item: unknown) => arr.filter((s) => s === item).length; if (answer.some((s) => counts(answer, s) > counts(q.options, s))) throw new ApiError(400, "Palavras repetidas inválidas."); result[q.id] = answer as string[]; }
  }
  return result;
}
function scoreQuestions(questions: Question[], answers: Answers) { const correct = questions.filter((q) => JSON.stringify(q.answer) === JSON.stringify(answers[q.id])).length; return { correct, total: questions.length, score: Math.round(correct / questions.length * 100) }; }
async function submit(auth: Auth, body: Body, diagnostic: boolean) {
  ...
  if (!diagnostic && body.practice !== true && catalog.some((a) => a.id === activity.id)) {
    ...
    const status = getWorldStatus(deriveStudentProgress(history.results.map(submissionRow)), activity.worldId);
    if (!status.unlocked) throw new ApiError(403, "Conclua as missões do mundo anterior para abrir esta etapa.", "WORLD_LOCKED");
  }
...
function cleanActivity(raw: unknown): Omit<CustomActivity, "id" | "createdAt" | "status" | "version"> {
  ...
  const questions = a.questions.map((rawQuestion, i): Question => {
    const q = obj(rawQuestion), type = q.type === "order" ? "order" : "choice";
    const id = q.id ? valueId(q.id, "Questão") : "q-" + (i + 1); if (questionIds.has(id)) throw new ApiError(400, "As questões precisam de identificadores diferentes."); questionIds.add(id);
    ...
    const options = q.options.map((o) => valueString(o, "Opção", 140));
    let answer: string | string[];
    if (type === "choice") { answer = valueString(q.answer, "Resposta", 140); if (!options.includes(answer)) throw new ApiError(400, "A resposta deve estar entre as opções."); }
    else { if (!Array.isArray(q.answer) || q.answer.length !== options.length || q.answer.some((s) => typeof s !== "string")) throw new ApiError(400, "A ordem correta deve incluir todas as palavras."); answer = q.answer as string[]; if (JSON.stringify([...answer].sort()) !== JSON.stringify([...options].sort())) throw new ApiError(400, "A ordem correta deve usar cada opção uma vez."); }
    return { id, type, prompt: valueString(q.prompt, "Enunciado", 800), options, answer, explanation: q.explanation ? valueString(q.explanation, "Explicação", 800) : "Vamos continuar aprendendo!", ...(q.stimulus ? { stimulus: valueString(q.stimulus, "Texto de apoio", 2000) } : {}), ...(q.audioText ? { audioText: valueString(q.audioText, "Narração", 2000) } : {}), ...(q.visual ? { visual: valueString(q.visual, "Ilustração", 20) } : {}) };
  });
...
export async function dispatch(auth: Auth, body: Body): Promise<Record<string, unknown>> {
  ...
  if (action === "assignActivity") {
    ...
    await notify(auth, "Nova missão da turma", activity.title + " · prazo " + dueDate); await audit(auth, action, activity.id); return {};
  }
  ...
  if (action === "publishActivity") {
    const row = await scopedRecord("activities", auth, body.id); cleanActivity(JSON.parse(String(row.body)));
    ...
    await audit(auth, action, String(row.id) + ":v" + row.version); return {};
  }
