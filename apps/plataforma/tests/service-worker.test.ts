...
function harness() {
  ...
  serve('/plataforma', '<html><script type="module">import("/assets/entry.js")</script><link href="/assets/style.css" rel="stylesheet"></html>', 'text/html');
...
test('download follows transitive and deferred build assets before making the shell available offline', async () => {
  ...
  for (const asset of ['/', '/plataforma', '/assets/entry.js', '/assets/game.js', '/assets/deferred.js', '/assets/style.css']) {
  ...
  app.disconnect();
  assert.match(await (await app.request('/plataforma', 'navigate'))!.text(), /entry\.js/);
...
test('an incomplete transfer never replaces an existing complete offline package', async () => {
  ...
  app.serve('/plataforma', '<html><script src="/assets/missing.js"></script></html>', 'text/html');
...
test('API, audio, identity, RSC, authorized, POST, range and external requests bypass the worker', async () => {
  const app = harness();
  for (const path of ['/api/platform', '/api/session', '/login', '/api/audio?id=private', '/signin-with-chatgpt', '/callback', '/private.js', '/?rsc=1', '/index.rsc', 'https://other.example/assets/game.js']) {
...
test('download messages cannot put private or external URLs into the public cache', async () => {
  const app = harness();
  for (const path of ['/api/platform', '/api/session', '/login', '/api/audio?id=private', 'https://other.example/assets/game.js', '/assets/game.js?token=private', '/@vite/client']) {
...
test('the first installation offers offline instructions and updates wait for an explicit choice', async () => {
  ...
  app.disconnect();
  assert.match(await (await app.request('/plataforma', 'navigate'))!.text(), /Prepare suas atividades/);
...
test('live navigation does not overwrite the downloaded shell with a partially loaded version', async () => {
  ...
  app.serve('/plataforma', '<html>New version</html>', 'text/html');
  assert.match(await (await app.request('/plataforma', 'navigate'))!.text(), /New version/);
  app.disconnect();
  assert.match(await (await app.request('/plataforma', 'navigate'))!.text(), /entry\.js/);
});
