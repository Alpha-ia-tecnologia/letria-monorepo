...
function harness(offline=false){
 ...
 const caches={open:async()=>cache,match:async(req:string|Request)=>storage.get(key(req)),keys:async()=>['letria-public-v2'],delete:async()=>true};
 const fetch=async(input:string|Request)=>{if(offline)throw new Error('offline');const url=key(input),path=new URL(url).pathname;downloads.push(url);const type=(path==='/'||path==='/plataforma')||path.endsWith('.html')?'text/html':path.endsWith('.css')?'text/css':path.endsWith('.png')?'image/png':path.endsWith('.webmanifest')?'application/manifest+json':path.endsWith('.woff2')?'font/woff2':'application/javascript';const body=(path==='/'||path==='/plataforma')?'<script src="/assets/app.js"></script>':path.endsWith('.css')?'@font-face{src:url("/fonts/test.woff2")}':'asset';return new Response(body,{headers:{'Content-Type':type}});};
...
test('RF-110: navegação offline recupera shell sem inventar dados',async()=>{
 const h=harness(true);h.storage.set('https://letria.test/plataforma',new Response('Letria offline'));let response:Promise<Response>|undefined;
 h.listeners.fetch({request:{url:'https://letria.test/plataforma',method:'GET',mode:'navigate',headers:new Headers()},respondWith:(r:Promise<Response>)=>{response=r;}});assert.equal(await(await response!).text(),'Letria offline');
});
