...
test("HTTP serves the public Letria landing page and product metadata without a session", { timeout: 60_000 }, async () => {
  ...
  assert.match(html, /Pequenas descobertas[.]/i);
  assert.match(html, /Grandes aventuras[.]/i);
...
test("HTTP serves the installable PWA manifest, icons and service worker", { timeout: 60_000 }, async () => {
  ...
  assert.equal(manifest.start_url, "/plataforma");
...
test('HTTP serves the full login, registration and student access pages with real form controls', { timeout: 60_000 }, async () => {
  for (const [path, controls] of [
    ['/login', ['email', 'password']],
    ['/login?modo=estudante', ['code']],
    ['/login?modo=cadastro', ['name', 'email', 'password']],
  ]) {
    const response = await fetch(new URL(path, testOrigin), { signal: AbortSignal.timeout(15_000) });
    assert.equal(response.status, 200, path);
    assert.equal(response.headers.get('set-cookie'), null, 'Opening login must not create a demonstration session.');
    const html = await response.text();
    for (const name of controls) assert.match(html, new RegExp('<input\\b[^>]*\\bname=["\']' + name + '["\']'), path + ': ' + name);
    assert.match(html, /<form\b/);
    assert.match(html, /href=["']\/["']/);
  }
});

test('HTTP serves the independent application shell for signed-in and explicit demo entry', { timeout: 60_000 }, async () => {
  for (const path of ['/plataforma', '/plataforma?demo=1']) {
    const response = await fetch(new URL(path, testOrigin), { signal: AbortSignal.timeout(15_000) });
    assert.equal(response.status, 200);
    const html = await response.text();
    assert.match(html, /Preparando a sua próxima descoberta/);
    assert.equal(response.headers.get('set-cookie'), null);
  }
});
