...
export function db(): Database { return database(); }
...
export async function authenticateAction(request: Request, body: Record<string, unknown>, previous: Auth | null): Promise<string | null> {
  ...
  if (action === "logout") { if (previous) await db().prepare("DELETE FROM sessions WHERE token_hash=?").bind(previous.tokenHash).run(); return null; }
  if (action === "register") {
...
export async function fetchActivity(auth: Auth, rawId: unknown, requestedVersion?: unknown) {
  const activityId = valueId(rawId, "Atividade"), builtIn = getCatalogActivity(activityId);
