...
export function evaluateAnswer(question: Question, answer: Answer | undefined): boolean {
  ...
  if (question.type === "multi" || question.type === "match") {
    if (!Array.isArray(answer) || !Array.isArray(question.answer)
      || !answer.length || answer.length !== question.answer.length
      || !answer.every(part => typeof part === "string")) return false;
    // These are selected labels, so case is meaningful (for example A versus a).
    const actual = answer;
    const expected = question.answer;
    const available = question.type === "match" ? question.matches ?? [] : question.options;
    if (new Set(actual).size !== actual.length || actual.some(part => !available.includes(part))) return false;
    return question.type === "multi"
      ? actual.every(part => expected.includes(part))
      : actual.length === question.options.length && actual.every((part, index) => part === expected[index]);
  }
...
export function getWorldStatus(progress: StudentProgress, worldId: number): WorldStatus {
  ...
  let reason: string;
  if (!unlocked) reason = "Conquiste os quatro territórios do mundo anterior para abrir este caminho.";
  else if (evidence.mastered) reason = "Você mostrou o que aprendeu em diferentes desafios. Continue pela trilha para abrir o próximo mundo.";
...
export function recordAttempt(progress: StudentProgress, input: {
  ...
  const next: StudentProgress = withStudyDay({ ...progress, attempts: [...progress.attempts, attempt],
...
export function completeActivity(progress: StudentProgress, activityId: string, now?: string | Date): StudentProgress {
  ...
  const earned = reward ? [...rewarded, activityId] : [...rewarded];
  const journey = getJourney({rewardedActivityIds: earned});
  return { ...next,
    unlockedWorldIds: [...new Set([...next.unlockedWorldIds, ...journey.worlds.filter(item => item.unlocked).map(item => item.world.id)])],
...
export function deriveStudentProgress(submissions: SubmissionEvidence[]): StudentProgress {
  ...
  for (const submission of chronological) {
    ...
    if (submission.activityId !== "diagnostic" && scoreActivity(submission.activityId, submission.answers).score >= 80) conquered.add(submission.activityId);
    // A territory requires a passing complete submission, not answers pieced together from failed attempts.
    progress.rewardedActivityIds = [...conquered];
    progress.unlockedWorldIds = getJourney(progress).worlds.filter(item => item.unlocked).map(item => item.world.id);
  }
...
export function canStartActivity(progress: StudentProgress, activityId: string): boolean {
  ...
  const activity = getActivity(activityId);
  return !!activity && getJourney(progress).nodes.some(node => node.activity.id === activityId && node.status !== "locked");
}
export function getRecommendation(progress: StudentProgress): Recommendation {
  const nextTerritory = getJourney(progress).next;
  if (nextTerritory) {
    const activity = nextTerritory.activity;
    return { activity, activityId: activity.id, worldId: activity.worldId, title: activity.title,
      kind: nextTerritory.index === 0 ? "start" : nextTerritory.index % 4 === 0 ? "advance" : "practice",
      reason: "Conquiste " + nextTerritory.title + " para abrir o próximo caminho. A Lumi pode ajudar com uma pista!" };
  }
