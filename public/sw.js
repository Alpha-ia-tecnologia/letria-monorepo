...
const CACHE_NAME = 'letria-public-v1';
...
function publicURL(value, base = self.location.origin) {
  ...
  try {
    ...
    if (url.pathname !== '/' && !PUBLIC_FILES.has(url.pathname) && !BUILD_ASSET.test(url.pathname)) return null;
    url.hash = '';
    return url.href;
  } catch {
    return null;
...
async function download(urls) {
  ...
  const pending = new Set(['/', ...INSTALL_FILES, '/og.png', ...requested].map(value => publicURL(value)));
  ...
  try {
    while (pending.size) {
      ...
      await Promise.all(batch.map(async url => {
        // '/' renders a generic loading shell; user records are fetched only
